...
postgres:
	docker run --name postgres12 --name bank-network -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=secret -d postgres:12-alpine
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/keshavbhattad/bank/db/sqlc Store
